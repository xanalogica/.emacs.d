...
** Use the ~diminish~ package to hide selected minor-modes from my modeline

The github:[[https://github.com/myrjola/diminish.el][diminish]] Elisp package can be used to remove or change minor mode
strings in your mode-line, by using the =:diminish <SYMBOL>= in your
~use-package~ invocation.  Diminished modes are minor modes with no modeline
display, because we don't want to be reminded of it.

#+BEGIN_SRC emacs-lisp
  (use-package diminish)
#+END_SRC

** Use the ~bind-key~ Elisp package to ???
...
;;                       "TAB" #'origami-toggle-node)
    :config ;; code to run after package has been loaded
      ...
      (defvar ap/org-super-agenda-auto-show-groups
        '("Finance" "Health" "ZeOmega" "Personal"))
