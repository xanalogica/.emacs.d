...
;;;           (org-agenda-add-entry-text-maxlines 5)  ;; experimental
       ...
       ("za" "Agenda for Current Period" agenda  ;; key[1] desc[2] type[3]
         nil  ;; entry match-expression[4]
         ( ; [5] list of settings local to this custom agenda view
           (org-agenda-overriding-header "Agenda for Each Day")
...
#+BEGIN_SRC emacs-lisp
  (defun jeff/notion-help() (interactive)
...
;;    (load-file "~/.emacs.d/init.el"))
    ...
    (let
      (
        (pos
          (save-excursion
            (find-file "~/Dropbox/Documents/Technology.org")
            (org-find-exact-headline-in-buffer "Notion Quick Reference")
          )
        )
      )
      (org-refile 4 nil "Notion Window Manager")
    )
  )
  (jeff/notion-help)
#+END_SRC
