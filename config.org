...
***** An Action to Take (collected, not filed yet; but may be relevant to a known team/project)
#+BEGIN_SRC emacs-lisp
#+END_SRC

***** Cool Quote to Remember (personal, zeomega)
#+BEGIN_SRC emacs-lisp
#+END_SRC
...
***** Information to Consume (book, movie, website)
#+BEGIN_SRC emacs-lisp
#+END_SRC

***** Information to Produce (whitepaper, blog post, diagram)
#+BEGIN_SRC emacs-lisp
#+END_SRC
...
;;;;  - Blood Pressure Measurement  (never refiled)
...
#+BEGIN_SRC emacs-lisp :results value :var keyseq="bp"
  (list keyseq "a Bloodpressure measurement" 'table-line
     (list 'file+headline "~/Dropbox/Documents/Org-Mode/Health.org"
                    "Jeff's Blood Pressure History")

     "| %u | %^{High} | %^{Low} | %^{Pulse} |"

     :prepend          ;; put newest measurements at the top of the table
     :unnarrowed       ;; don't narrow to just the new entry; I want to see my history
     :kill-buffer      ;; if file was opened just to collect data, close the buffer after
  )
#+END_SRC

#+RESULTS: health-bloodpressure-measurements
| bp | a Bloodpressure measurement | table-line | (file+headline ~/Dropbox/Documents/Org-Mode/Health.org Jeff's Blood Pressure History) |   | %u | %^{High} | %^{Low} | %^{Pulse} |   | :prepend | :unnarrowed | :kill-buffer |

#+CALL: health-bloodpressure-measurements(keyseq="bb")

#+RESULTS:
| bb | a Bloodpressure measurement | table-line | (file+headline ~/Dropbox/Documents/Org-Mode/Health.org Jeff's Blood Pressure History) |   | %u | %^{High} | %^{Low} | %^{Pulse} |   | :prepend | :unnarrowed | :kill-buffer |


***** TODO FICO Credit Score Monthly Measurement

#+BEGIN_SRC emacs-lisp
;;;;  - A New FICO Monthly Score    (never refiled)
;;       ("f" "a [F]ICO measurement" table-line
;;         (file+headline "~/notes/fitness.org" "Blood Pressure")
;;         "| %t | %^{High} | %^{Low} | %^{Pulse} |"
...
Assemble the various definitions of my capture templates.

#+BEGIN_SRC emacs-lisp
  (setq org-capture-templates
    '(
       (health-bloodpressure-measurements "bp")

       ("z" "ZeOmega Capture Templates")
     )
  )
...
#+RESULTS:
| (health-bloodpressure-measurements bp) | (z ZeOmega Capture Templates) | - | meeting | notes |

;;;;;;;; HOW TO INCLUDE: Blood Pressure Measurement
;;;;;;;; HOW TO INCLUDE: FICO Credit Score Monthly Measurement
...
;;;;;       ("q" "a [Q]uotation to remember" entry
;;;;;         (file+headline "~/notes/quotes.org" "Disorganized Quotations")
;;;;;         "* %U :quotation:REFILE:\n\n%i%?\n"
;;;;;         :prepend       t       ;; prepend new item to top of capture target section
;;;;;         :empty-lines 1         ;; count of blank lines before and after
;;;;;       )
