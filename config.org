...
DONE add in prompt for title %^{prompt}
DONE add in prompt for tags  %^G
auto-insert what is in the clipboard  %i
DONE position in body for additional notes
support refiling bookmarks (from selected text)
...
#+BEGIN_SRC text
  ...
  :prepend t         ;; put newest quote at the top of the section
  :kill-buffer t     ;; if file was opened just to collect data, close the buffer after
  :empty-lines-before 1     ;; enforce count of blank lines before and after
  :empty-lines-after 1     ;; enforce count of blank lines before and after
#+END_SRC
