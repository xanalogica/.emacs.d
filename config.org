...
#+BEGIN_SRC emacs-lisp keyseq="bp"
...
:kill-buffer
(a Bloodpressure measurement table-line (file+headline ~/Dropbox/Documents/Org-Mode/Health.org Jeff's Blood Pressure History) | %u | %^{High} | %^{Low} | %^{Pulse} | :prepend :unnarrowed :kill-buffer)
...
#+BEGIN_SRC emacs-lisp :results code
  (setq org-capture-templates
    (list
       (list "bp" (health-bloodpressure-measurements))
