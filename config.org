...
;;;        )
      ...
      (save-mark-and-excursion
        ...
        ; Step 1: Ascend tree to find nearest headline with :DIRECTION: tag

        (while
          (not (member "DIRECTION" (org-element-property :tags (org-element-at-point))))
;;          (progn
;;            (setq headline (org-element-at-point))
;;            (not (member "DIRECTION" (org-element-property :tags headline)))
;;          )
          (outline-up-heading 1)
        ) ; end of while
        (setq result (format "%S" headline))

        ; Step 2: Capture a Link to Headline At Which We're Now Pointing
        (setq result (make-link-to-here))
      )

      (setq match "+INITIATIVE+DIRECTION={RD-41}")
