...
;;;           (org-agenda-add-entry-text-maxlines 5)  ;; experimental
       ...
       ("zb" "Backlog of Unscheduled Tasks" todo  ;; key[1] desc[2] type[3]
         nil  ;; entry match-expression[4]
         ( ; [5] list of settings local to this custom agenda view
...
#+BEGIN_SRC emacs-lisp
  (setq groups/zeomega-tech-directions  ;; presentation defn using org-super-agenda
    '(
...
#+BEGIN_SRC emacs-lisp
  (defun jeff--auto-group-by-multiprop (all-items propname &rest args)
    ...
    (list ':auto-multiprop non-matching ngroups)
  )
...
;;        (setq result (make-link-to-here)) ;; capture a link to that headline
        (setq patt
...
;;      (setq patt (format "%S" patt))

      (setq patt "epic:RD-41\\\\]")

;;      (setq patt (org-make-org-heading-search-string patt))
      (setq patt (regexp-quote patt))
      (setq match (format "+INITIATIVE+DIRECTION={%s}" patt))


      ;;TBD fix construction of match expression                     
;;      (setq match "+INITIATIVE+DIRECTION={RD-46}")

;;      (setq patt "[[file:directions.org::*%5B%5Bhttps://karmic.zeomega.com/browse/RD-41%5D%5Bepic:RD-41%5D%5D%20Improve%20Jiva%20Deployability][RD-41 Improve Jiva Deployability]]")

;;      (setq patt "\\[\\[file:directions.org::\\*%5B%5Bhttps:\/")

;;      (setq patt "RD-41 Improve Jiva Deployability")           ;; WORKS
;;      (setq matchexpr (format "+INITIATIVE+DIRECTION={%s}" patt))

      (setq matcher (org-make-tags-matcher matchexpr)
            match   (car matcher)
            matcher (cdr matcher)
      )
...
;;      (org-set-sorting-strategy 'tags)
      ...
      (while (setq file (pop files))
        (catch 'nextfile
          ...
          (if (not buffer) ;; If file does not exist, error message to agenda
            (setq rtn
              (list
...
;;                    (org-scan-tags  ;; versus org-map-entries
;;                      'make-link-to-here           ;; ACTION
;;                      matcher                      ;; MATCHER
...
;;                    )
  ...
  ;;TBD: build a function that operates on each agenda file
...
1. fix code that builds MATCH expression
2. remove repeated prompts in collection of links

;; +INITIATIVE+DIRECTION="[[file:directions.org::*%5B%5Bhttps://karmic.zeomega.com/browse/RD-41%5D%5Bepic:RD-41%5D%5D%20Improve%20Jiva%20Deployability][RD-41 Improve Jiva Deployability]]"
;; +INITIATIVE+DIRECTION={RD-41}
(org-tags-iew TODO-ONLY MATCHSTRING)

        ;; does 'here' have a :DIRECTION: tag?
        ;; if so, exit loop
        ;; if not, set here to :parent and loop

;        (setq here (org-element-context))
;        (setq ancestors (org-element-lineage here))

;;        (setq thing (org-entry-get (point) "ISSUE-TYPE" t))
;; (org-element-type thing)
;; (org-element-property :parent thing)
;; 'headline
