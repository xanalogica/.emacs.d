...
;;      (:children t)
;;      (:auto-parent t)
;;      (:auto-map jeff--auto-group-by-direction)
;;      (:order 99 )

      (:order-multi (99 (:auto-direction t)))
;;      (:auto-direction :order 99)

;;      (:auto-property "DIRECTION")
;;      (:name "Tasks To Work On TODAY" :and (:tag "HOT" :tag "ZEOMEGA"))
      (:name "Administrative Chores  :CHORE:"
         :and (:tag "CHORE" :tag "ZEOMEGA"))
      (:name "Administrative Chores  :ADMIN:"
         :and (:tag "ADMIN" :tag "ZEOMEGA"))

      (:name "Corporate Events       :EVENT:"
         :and (:tag "EVENT" :tag "ZEOMEGA"))
      (:name "Corporate Travel       :TRAVEL:"
         :and (:tag "TRAVEL" :tag "ZEOMEGA"))
      (:name "Engage with People     :PEOPLE:"
         :and (:tag "PEOPLE" :tag "ZEOMEGA"))

      (:name "To Share My Knowledge  :TOSHARE:"
         :and (:tag "TOSHARE" :tag "ZEOMEGA"))
      (:name "To Share My Knowledge  :WRITING:"
         :and (:tag "WRITING" :tag "ZEOMEGA"))

      (:name "Things I Need to Study :TOSTUDY:"
         :and (:tag "TOSTUDY" :tag "ZEOMEGA"))
      (:name "Things I Need to Study :STUDY:"
         :and (:tag "STUDY" :tag "ZEOMEGA"))
...
;;  (defun XXjeff--auto-group-by-direction (item)
;;    (-when-let*
;;      (
;;        (marker (or (get-text-property 0 'org-marker item) (get-text-property 0 'org-hd-marker item)))
...
;;  (defun YYjeff--auto-group-by-direction (item)
;;    (-when-let*
;;      (
;;        (marker (or (get-text-property 0 'org-marker item) (get-text-property 0 'org-hd-marker item)))
;;        (value
...
;;          )
;;        )
;;        (values (mapcar #'org-entry-restore-space values))
;;      )
;;      (format "Direction: %s" (car values))
;;    )
;;  )
...
#+BEGIN_SRC emacs-lisp
  (defun jeff--auto-group-by-direction (all-items &rest args)
    ...
    (dolist (item all-items)
      ...
      (setq propvalue (org-entry-get marker "DIRECTION"))
      (if propvalue
        (let*
          (
            (directions
        ...
        (push item non-matching)  ;; else no DIRECTION property
      )
    )
    ...
    (list ':auto-direction non-matching ngroups)
  )

  (setq org-super-agenda-group-types
    (plist-put org-super-agenda-group-types
      ':auto-direction
      'jeff--auto-group-by-direction
    )
  )

  (add-to-list
    'org-super-agenda-auto-selector-keywords ':auto-direction)
#+END_SRC
