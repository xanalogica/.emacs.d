...
****** Emacs Capture Templates and Targets                                 :CAPTURETARGETS:
...
***** Emacs Refiling Targets                                                :REFILETARGETS:
...
#+BEGIN_SRC emacs-lisp
  (setq org-refile-targets
    '( ; file(s) to look inside of                  ; destination headings
      ...
      ("~/.emacs.d/config.org" :tag . "REFILETARGETS")
        ;; not really for refiling -to- but for jumping to easily
      ("~/.emacs.d/config.org" :tag . "CAPTURETARGETS")
        ;; not really for refiling -to- but for jumping to easily
...
***** My Custom Org-Agenda Views (defined on C-c a <KEY>)
    ...
    '(
       ("P" "Agenda for Current Period" agenda  ;; key[1] desc[2] type[3]
         ...
         ( ; [5] list of settings local to this custom agenda view
           ...
           (org-super-agenda-groups groups/scheduled-tasks)
...
;;;           (org-agenda-add-entry-text-maxlines 5)  ;; experimental
       ...
       ("F" "Captured Items Needing Filing" tags  ;; key[1] desc[2] type[3]
         ...
         ("/tmp/CapturedItems.pdf")  ;; file(s)-to-export[6]
       )
       ...
       ("zP" "Agenda for Current Period" agenda  ;; key[1] desc[2] type[3]
         nil  ;; entry match-expression[4]
         ( ; [5] list of settings local to this custom agenda view
           (org-agenda-overriding-header "Agenda for Each Day")
...
groups/zeomega-scheduled-tasks


       ("zD" "Directions of Technology for ZeOmega" tags  ;; key[1] desc[2] type[3]
         "+ALLTAGS={TechDirection}-NOSEARCH=1"  ;; entry match-expression[4]
         ( ;; [5] list of settings local to this custom agenda view
...
#+BEGIN_SRC emacs-lisp
  (setq groups/zeomega-scheduled-tasks  ;; presentation definition using org-super-agenda
    '(
      (:name "Tasks Scheduled for TODAY" :tag ("HOT" "ZEOMEGA"))
      (:name "Maintain Physical and Mental Health" :tag ("HEALTH" "ZEOMEGA"))
      (:name "To Not Worry about Money" :tag ("FINANCE" "ZEOMEGA"))
      (:name "To Take Care of Life's Little Chores" :tag ("CHORE" "ZEOMEGA"))
      (:name "To Experience Life's Magic" :tag ("EVENT" "ZEOMEGA"))
      (:name "To Explore the World" :tag ("TRAVEL" "ZEOMEGA"))
      (:name "To Stay in Touch with Others" :tag ("PEOPLE" "ZEOMEGA"))
      (:name "To Share My Knowledge with Others" :tag ("WRITING" "ZEOMEGA"))
      (:name "To Continually Learn New Things" :tag ("STUDY" "ZEOMEGA"))
      (:name "To Have a Fantastic Career" :tag "ZEOMEGA")
      ;; After the last group, the agenda will display items that didn't
      ;; match any of these groups, with the default order position of 99
    )
