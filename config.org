...
#+BEGIN_SRC emacs-lisp
  ...
  (defun jeff--auto-group-by-direction (all-items &rest args)
    ...
    (while all-items       ;; (dolist (item allitems) ...)
      (setq item (pop all-items))
...
;;            (directions
;;              (mapcar #'org-entry-restore-space directions)
;;            )
          )
          (while directions
            (setq direction (pop directions))

            (ht-set! groups value (cons item (ht-get groups value)))
          )
        )
...
;;      (format "Direction: %s" (car values))
    ...
    (setq ngroups nil)
    (setq keys (sort (ht-keys groups) 'string<))
    (while keys
      (setq key (pop keys))
...
(org-super-agenda--def-auto-group )
   group 'key' should be each distinct direction
